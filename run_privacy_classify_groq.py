# -*- coding: utf-8 -*-
"""
互联网问诊文本 —— 实体抽取与分级（Groq 版，一体化脚本）
- 稳健 .env 加载（脚本同目录优先，其次向上查找），支持 gsk_/grq_ 前缀
- 严格 JSON 输出 + 字段名归一化（别名→枚举、模糊匹配）
- 调试日志：前若干次打印 LLM 原始 JSON，便于定位“全是 []”的问题
- 两段式重试：第一次失败后，追加“字段必须用枚举”强提醒再试
- HPV 高危阳性/特殊病种 升级规则
- 长文本分块、去重合并
- Excel 导出两张表（行级汇总 / 实体明细）
"""
import os, sys, time, json, re, difflib, random
...
# ============== 全局/配置 ==============
# —— 模型 & 任务配置（可按需在 .env 覆盖）——
MODEL = os.getenv("GROQ_MODEL", "DeepSeek-R1-Distill-Llama-70B")  #"llama-3.3-70b-versatile","llama-3.1-8b-instant","qwen/qwen3-32b","moonshotai/kimi-k2-instruct-0905","llama-3.3-70b-versatile"
...
SLEEP_SEC = 0.6
DEBUG_FIRST_N = 100  # 只跑前 N 行；全量设为 None
...
OUTPUT_FILE = f"{os.path.splitext(INPUT_FILE)[0]}_classified_entities(DeepSeek-R1-Distill-Llama-70B)_{FEWSHOT_MODE}.xlsx" #{MODEL}

# 原始输出调试：仅打印前 K 次调用的 Raw JSON
VERBOSE_LOG_FIRST_K = 3
...
# 注意：上面与原始版本略有修正：把“血压/血氧饱和度”与“生命体征-xxx”保持一致，避免模型/别名分歧
...
def _pick_examples(examples: List[Dict[str, Any]], topk=6, mode="first"):
    if not examples:
        return []
    if mode == "random":
...
def build_messages_and_schema(text: str, max_items: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    response_format = {"type": "json_object"}  # Groq 的 JSON 模式

    return messages, {"schema": schema_core, "response_format": response_format}
...
def _validate_and_coerce(obj: Dict[str, Any], schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 限量 & 排序
    out["items"] = out["items"][: schema["properties"]["items"]["maxItems"]]
...
def call_once(text: str, max_items: int) -> Dict[str, Any]:
    _call_counter["n"] += 1
    messages, pack = build_messages_and_schema(text, max_items)
    schema_core = pack["schema"]
    ...
    try:
        ...
        if _call_counter["n"] <= VERBOSE_LOG_FIRST_K:
            print(f"[call_once.A] Groq 异常: {e}")

    # 路径 B：强提醒字段必须用枚举，再试一次
    try:
        fix_msgs = list(messages) + [{
            "role":"user",
            "content":(
                "上面的回答字段名不在给定枚举里被丢弃了。"
                "请严格从枚举中选择 field 值（原样拷贝），不要创造新字段名；"
                "如果拿不准，就选“最接近”的一个枚举项。仅输出 JSON。"
            )
        }]
        ...
        raw2 = comp2.choices[0].message.content or ""
        if _call_counter["n"] <= VERBOSE_LOG_FIRST_K:
            print("\n[RAW fallback]\n", raw2[:1200], ("\n... [truncated]" if len(raw2) > 1200 else ""))
        obj2 = _parse_json(raw2)
        return _validate_and_coerce(obj2, schema_core)
    except Exception as e:
        if _call_counter["n"] <= VERBOSE_LOG_FIRST_K:
            print(f"[call_once.B] Groq 异常: {e}")
        return {"items": [], "max_risk_level": 0}
...
# ============== 入口 ==============
def main():
    # —— 稳健加载 .env ——（脚本同目录优先，其次从当前工作目录向上查找）
    base_dir = Path(__file__).resolve().parent
    local_env = base_dir / ".env"
    auto_env = find_dotenv(usecwd=True)
    env_path = str(local_env) if local_env.exists() else (auto_env or None)
    print(f"[dotenv] using: {env_path or 'NOT FOUND'}")
    load_dotenv(dotenv_path=env_path, override=True)

    api_key = os.getenv("GROQ_API_KEY")
    print("[dotenv] GROQ_API_KEY prefix:", (api_key or "")[:4])  # 仅打印前缀，避免泄露
    assert api_key, (
        "未读到 GROQ_API_KEY。\n"
        "请检查：1) .env 与脚本同目录或项目根目录；2) 文件名 '.env'；3) 格式 GROQ_API_KEY=gsk_xxx（不加引号）。"
    )
    if not (api_key.startswith("gsk_") or api_key.startswith("grq_")):
        print("⚠️ 提示：Groq 密钥通常以 gsk_ 开头")
